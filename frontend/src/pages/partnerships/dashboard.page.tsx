...
export default function DashboardPage() {
  ...
  const fetchApplication = async () => {
    try {
      ...
      const currentStatus = newApplication.paymentTracking?.status;

      setApplication(newApplication);

      // If payment status changed to "received", show notification and update user context
      if (
        ...
        addToast(
          "✓ Payment received! Partnership status updated.",
          "success",
          4000,
        );

        // Update user's partner status in context if converted to partner
        if (newApplication.paymentTracking?.userConvertedToPartner && user) {
          setUser({
            ...user,
            isPartner: true,
          });
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      // Only refetch when tab becomes visible
      if (document.visibilityState === "visible") {
        ...
        // Wait 500ms before refetching to avoid multiple simultaneous requests
        // if user rapidly switches tabs
                            ...
                            <div className="bg-white border border-gray-300 rounded-2xl p-8 mb-8">
                              ...
                              <div className="flex flex-col sm:flex-row gap-4 items-stretch mb-6">
                                <textarea
                                  ...
                                  rows={5}
                                  className="flex-1 px-4 py-3 bg-[#f5f5f0] border border-gray-300 rounded-lg text-[#1a1a1a] font-medium resize-none overflow-y-auto whitespace-pre-wrap break-words"
                                />
                                {/* <button
                                onClick={() => {
                          ...
                          <div className="bg-white border border-gray-300 rounded-2xl p-8">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-[#6b6b6b] text-sm">
                                  Your admin is preparing the payment details
                                  for the{" "}
                      ...
                      <div className="bg-white rounded-2xl p-8 md:p-12 border border-gray-200 text-center space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-6">
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="text-4xl md:text-5xl font-black text-[#d4af37]">
                              {formatCurrency(
                                application.projectedAnnualReturn,
                              )}
                            </p>
                          </div>
                          <div className="space-y-2">
